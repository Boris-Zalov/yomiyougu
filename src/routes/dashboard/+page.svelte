<script lang="ts">
  ...
  import { BookOpenOutline, ClockOutline, ArrowRightOutline, PlusOutline, DownloadOutline, ExclamationCircleOutline } from "flowbite-svelte-icons";
  ...
  let recentlyReading = $derived.by(() => {
    const readingBooks = allBooks.filter(b => 
  ...
  async function handleDownloadConfirm() {
    ...
    try {
      ...
      allBooks = allBooks.map(b => b.id === updatedBook.id ? { ...b, file_path: updatedBook.file_path } : b);
      ...
      <div class="flex flex-col items-center gap-4">
        ...
        <div>
          ...
          <P class="text-gray-500 dark:text-gray-400 max-w-md mx-auto">
            Import some manga or comics to get started. Your reading progress will be tracked here.
          </P>
        </div>
    ...
    <Card class="text-center py-12 px-6">
      <div class="flex flex-col items-center gap-4">
        ...
        <div>
          ...
          <P class="text-gray-500 dark:text-gray-400 max-w-md mx-auto">
            You have {totalBooks} {totalBooks === 1 ? 'book' : 'books'} in your library. Pick one to start reading!
          </P>
        </div>
        <Button href="/library" class="mt-2">
  ...
  {#if recentlyReading.length > 0}
    ...
    <section>
      <div class="flex items-center justify-between mb-4">
        ...
        <Button href="/library?status=reading" size="sm" color="alternative" class="hidden sm:flex">
          View All
