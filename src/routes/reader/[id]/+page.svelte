<script lang="ts">
	...
	import { Modal, Button, Label, Input, Textarea, Spinner, Drawer } from "flowbite-svelte";
	import {
		CloseOutline,
		BookmarkSolid,
		HeartSolid,
	...
	async function loadData() {
		...
		try {
			...
			const themeValue = settings.categories
				...
				?.settings.find((s) => s.key === "appearance.theme")?.value as ThemeMode | undefined;
			isDarkMode = getEffectiveTheme(themeValue || "system") === "dark";

			const readingCategory = settings.categories.find((c) => c.id === "reading");

			const defaultDirection =
				(readingCategory?.settings.find((s) => s.key === "reading.direction")?.value as string) ??
				"rtl";
			const defaultDisplayMode =
				(readingCategory?.settings.find((s) => s.key === "reading.page_display_mode")
					?.value as string) ?? "single";
			const defaultFitMode =
				(readingCategory?.settings.find((s) => s.key === "reading.image_fit_mode")
					?.value as string) ?? "fit_width";

			readingDirection = (bookSettings?.reading_direction ??
				defaultDirection) as typeof readingDirection;
			pageDisplayMode = (bookSettings?.page_display_mode ??
				defaultDisplayMode) as typeof pageDisplayMode;
	...
	async function updateBookSetting(key: keyof BookSettings, value: string | boolean) {
		...
		try {
			...
			const switchedToContinuous =
				previousPageDisplayMode !== "continuous" && pageDisplayMode === "continuous";
			const switchedToVertical =
				previousReadingDirection !== "vertical" && readingDirection === "vertical";
...
{:else}
	...
	>
		...
		{#if isContinuous}
			...
			>
				{#each Array(totalPages) as _, pageIndex}
					<div data-page-index={pageIndex} class="flex items-center justify-center w-full">
						<img
		...
		{:else if isDouble}
			<div class="h-full w-full flex items-center justify-center">
				...
				{/if}
				<div
					class="h-full flex items-center justify-center gap-1 {readingDirection === 'rtl'
						? 'flex-row-reverse'
						: 'flex-row'}"
				>
		...
		{#if !isVertical && !isContinuous && !showOverlay}
			<div
				class="absolute inset-y-0 left-0 w-[30%] flex items-center justify-start pl-4 opacity-0 hover:opacity-30 transition-opacity pointer-events-none"
			>
				{#if readingDirection === "rtl"}
			...
			<div
				class="absolute inset-y-0 right-0 w-[30%] flex items-center justify-end pr-4 opacity-0 hover:opacity-30 transition-opacity pointer-events-none"
			>
				{#if readingDirection === "rtl"}
					<ChevronLeftOutline class="w-12 h-12 text-white drop-shadow-lg" />
				{:else}
	...
	<div
		class="fixed bottom-0 left-0 right-0 h-1 bg-black/30 z-30 {readingDirection === 'rtl'
			? 'flex justify-end'
			: ''}"
	>
		<div
	...
	{#if showOverlay}
		...
		>
			...
			<div
				class="fixed top-0 left-0 right-0 p-4 flex items-center justify-between z-50 {isDarkMode
					? 'bg-gray-900/90'
					: 'bg-white/90'}"
				...
				{#if isAndroid}
					...
					<div class="flex items-center gap-2 mt-7">
						{#if isVertical}
							...
							<button
								class="text-sm font-medium px-2 hover:underline {isDarkMode
									? 'text-white'
									: 'text-gray-900'}"
								onclick={(e) => {
									e.stopPropagation();
						...
						{:else}
							...
							>
								<ChevronRightOutline
									class="w-6 h-6 {isDarkMode ? 'text-white' : 'text-gray-900'}"
								/>
							</button>
						{/if}
					</div>
				...
				{:else}
					...
					<div class="flex items-center gap-4">
						<span
							class:text-white={isDarkMode}
							class:text-gray-900={!isDarkMode}
							class="font-medium truncate max-w-[200px] md:max-w-none"
						>
							{book?.title ?? "Loading..."}
						</span>
					</div>
			...
			{#if isAndroid}
				...
				<!-- svelte-ignore a11y_click_events_have_key_events -->
				<div
					class="fixed bottom-0 left-0 right-0 p-4 flex items-center justify-between z-50 {isDarkMode
						? 'bg-gray-900/90'
						: 'bg-white/90'}"
					onclick={(e) => e.stopPropagation()}
					role="toolbar"
			...
			{:else}
				...
				<button
					class="fixed bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full z-50 hover:ring-2 hover:ring-primary-500 transition-all {isDarkMode
		...
		<div class:opacity-50={isContinuous}>
			...
			<div class="grid grid-cols-3 gap-2">
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {readingDirection === 'ltr'
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {readingDirection === 'rtl'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
					disabled={isContinuous}
				>
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {readingDirection === 'vertical'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
					disabled={isContinuous}
				>
		...
		<div>
			...
			<div class="grid grid-cols-3 gap-2">
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {pageDisplayMode === 'single'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Single
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {pageDisplayMode === 'double'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Double
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {pageDisplayMode === 'continuous'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Continuous
		...
		<div>
			...
			<div class="grid grid-cols-2 gap-2">
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {imageFitMode === 'fit_width'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Fit Width
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {imageFitMode === 'fit_height'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Fit Height
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {imageFitMode === 'fit_screen'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Fit Screen
				...
				<button
					...
					class="px-3 py-2 text-xs rounded transition-colors {imageFitMode === 'original'
						? 'bg-primary-600 text-white'
						: 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}"
				>
					Original
...
<Modal bind:open={showBookmarkModal} size="sm" autoclose={false}>
	<div class="space-y-4">
		...
		<div>
			...
			<Input id="bookmark-name" bind:value={bookmarkName} placeholder="Bookmark name" />
		</div>
		<div>
		...
		<div class="flex gap-2 justify-end mt-4">
			...
			<Button onclick={saveBookmark} disabled={!bookmarkName.trim()}
				>{editingBookmark ? "Save" : "Create"}</Button
			>
		</div>
	</div>
</Modal>
...
{#if showToast}
	<div class="fixed bottom-20 left-1/2 -translate-x-1/2 z-50">
		<div
			class="flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg {toastType === 'success'
