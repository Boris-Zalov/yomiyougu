<script lang="ts">
	...
	function sortBooks(items: BookWithDetails[]): BookWithDetails[] {
		...
		switch (bookSortField) {
			...
			case "date_added":
				sorted.sort(
					(a, b) => dir * (new Date(a.added_at).getTime() - new Date(b.added_at).getTime())
				);
				break;
			case "last_read":
				sorted.sort((a, b) => {
	...
	let collectionsFuseCache: {
		data: CollectionWithCount[];
		fuse: Fuse<CollectionWithCount>;
	} | null = null;
			...
			if (result.success) {
				...
				try {
					...
					const theme = (settings.categories
						...
						?.settings.find((s) => s.key === "appearance.theme")?.value || "system") as ThemeMode;
					applyTheme(theme);
				} catch (e) {
...
{:else}
		...
		<div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3">
          ...
          "
			...
			{#each filteredCollections as collection (collection.id)}
				<CollectionItem {collection} ondelete={confirmDeleteCollection} />
			{:else}
				{#if search.trim()}
	...
	{#if showCollectionModal}
		<Modal bind:open={showCollectionModal} size="md" class="w-full">
			<form
				...
				class="space-y-4"
			>
				<Heading tag="h3" class="text-lg font-medium">Create New Collection</Heading>
				...
				<div class="flex gap-3 pt-2">
					...
					<Button type="submit" color="primary" class="flex-1" disabled={isCreatingCollection}>
	...
	{#if showResultModal}
		<Modal bind:open={showResultModal} size="md" autoclose>
			<div class="text-center">
				...
				<Heading tag="h3" class="mb-5 text-lg font-normal">Import Complete</Heading>
	...
	{#if showErrorModal}
		<Modal bind:open={showErrorModal} size="md" autoclose>
			<div class="text-center">
				...
				<Heading tag="h3" class="mb-5 text-lg font-normal">Error</Heading>
	...
	{#if showDeleteBookModal}
		<Modal bind:open={showDeleteBookModal} size="md">
			<div class="text-center">
				...
				<Heading tag="h3" class="mb-2 text-lg font-medium">Delete Book</Heading>
				<P size="sm" class="mb-5 text-gray-500 dark:text-gray-400">
					Are you sure you want to delete "<strong>{bookToDelete?.title}</strong>"? This action
					cannot be undone.
				</P>
				<div class="flex gap-3">
					...
					<Button color="red" class="flex-1" onclick={handleDeleteBook} disabled={isDeleting}>
						{#if isDeleting}
							<Spinner size="4" class="mr-2" />
							Deleting...
						{:else}
	...
	{#if showDeleteCollectionModal}
		<Modal bind:open={showDeleteCollectionModal} size="md">
			<div class="text-center">
				<TrashBinOutline class="mx-auto mb-4 w-12 h-12 text-red-500 dark:text-red-400" />
				<Heading tag="h3" class="mb-2 text-lg font-medium">Delete Collection</Heading>
				<P size="sm" class="mb-5 text-gray-500 dark:text-gray-400">
					Are you sure you want to delete the collection "<strong>{collectionToDelete?.name}</strong
					>"? Books in this collection will not be deleted.
				</P>
				<div class="flex gap-3">
					<Button
						color="alternative"
					...
					<Button color="red" class="flex-1" onclick={handleDeleteCollection} disabled={isDeleting}>
						{#if isDeleting}
							<Spinner size="4" class="mr-2" />
							Deleting...
						{:else}
