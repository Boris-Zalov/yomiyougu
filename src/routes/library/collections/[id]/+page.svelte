<script lang="ts">
	...
	import { Heading, Button, Search, Hr, Modal, Spinner, Toast, P } from "flowbite-svelte";
	import {
		ArrowLeftOutline,
		CloseCircleSolid,
		PlusOutline,
	...
	<div class="page-container p-4">
		<div class="mb-6">
			<div class="flex items-center gap-4 mb-4">
				<Button href="/library" color="alternative" class="p-2.5!" aria-label="Back to library">
			...
			<Search clearable bind:value={search} placeholder={`Search in ${collection.name}...`} />
		</div>
...
<Modal bind:open={showAddModal} size="sm" class="w-full">
	<div class="space-y-4">
		<h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Add Book to Collection</h3>

		<Button color="light" class="gap-3 p-4 w-full">
			<a
				href={`/library/collections/${collectionId}/add-existing`}
		...
		>
			...
			<div class="text-left">
				...
				<div class="text-xs text-gray-500 dark:text-gray-400">Import a new archive file</div>
			</div>
		</Button>
	</div>
</Modal>
...
<Modal bind:open={showResultModal} size="md" class="w-full">
	<div class="text-center">
		...
		{/if}
		<Button color="green" onclick={() => (showResultModal = false)}>Continue</Button>
	</div>
</Modal>
...
<Modal bind:open={showErrorModal} size="md" class="w-full">
	<div class="text-center">
		...
		<h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Import Failed</h3>
		<p class="mb-5 text-sm text-gray-600 dark:text-gray-300">
			{errorMessage}
		</p>
		<Button color="red" onclick={() => (showErrorModal = false)}>Close</Button>
	</div>
</Modal>
...
<Modal bind:open={showDeleteBookModal} size="md" autoclose={false}>
	<div class="text-center">
		...
		<h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Delete Book</h3>
		{#if bookToDelete}
			<P class="mb-6 text-sm text-gray-600 dark:text-gray-300">
				Are you sure you want to delete <strong>{bookToDelete.title}</strong>? This action cannot be
				undone.
			</P>
		{/if}
		<div class="flex justify-center gap-4">
			<Button color="red" onclick={handleDeleteBook} disabled={isDeleting}>
				{#if isDeleting}
...
<Modal bind:open={showRemoveFromCollectionModal} size="md" autoclose={false}>
	<div class="text-center">
		...
		{#if bookToRemove && collection}
			<P class="mb-6 text-sm text-gray-600 dark:text-gray-300">
				Remove <strong>{bookToRemove.title}</strong> from <strong>{collection.name}</strong>? The
				book will remain in your library.
			</P>
		{/if}
		<div class="flex justify-center gap-4">
			<Button color="yellow" onclick={handleRemoveFromCollection} disabled={isRemoving}>
				{#if isRemoving}
					<Spinner size="4" class="mr-2" />
				{/if}
				Remove
...
<Modal bind:open={showCloudDownloadModal} size="md">
	<div class="text-center">
		...
		<P size="sm" class="mb-5 text-gray-500 dark:text-gray-400">
			"<strong>{pendingBook?.title}</strong>" is stored in the cloud. Would you like to download it
			to read?
		</P>
