<script lang="ts">
	...
	import { Heading, Button, Search, Hr, Spinner, Toast } from "flowbite-svelte";
	import {
		ArrowLeftOutline,
		FileCheckSolid,
		CheckCircleSolid,
	...
	<div class="page-container p-4">
		<div class="mb-6">
			<div class="flex items-center gap-4 mb-4">
				...
				{#if selectedBookIds.size > 0}
					<Button onclick={addSelectedBooks} disabled={isAdding} color="primary">
						{#if isAdding}
							<Spinner size="4" class="mr-2" />
						{/if}
						Add {selectedBookIds.size}
						{selectedBookIds.size === 1 ? "book" : "books"}
					</Button>
				{/if}
			</div>

			<Search clearable bind:value={search} placeholder="Search available books..." />
		</div>
		...
		<div class="flex items-center justify-between mb-4">
			<span class="text-sm text-gray-500 dark:text-gray-400">
				...
				{availableBooks.length === 1 ? "book" : "books"} available
				{#if selectedBookIds.size > 0}
		...
		<div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3">
            ...
            {isSelected(book.id)
						...
						: 'border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-700'} bg-gray-200 dark:bg-gray-700"
				>
					{#if isSelected(book.id)}
						<div
							class="absolute top-2 left-2 z-20 p-1.5 rounded-full bg-primary-500 text-white shadow-lg"
						>
              ...
              {isSelected(book.id) ? 'opacity-80' : ''}"
					...
					<div
						class="absolute bottom-0 left-0 right-0 z-10 p-2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-t border-gray-100 dark:border-gray-700"
					>
						<h5
							class="text-xs font-bold text-gray-900 dark:text-white line-clamp-2 leading-tight text-left"
							title={book.title}
						>
							{book.title}
						</h5>
			...
			{:else}
				...
				{:else}
					<div
						class="col-span-full py-12 text-center bg-gray-50 dark:bg-gray-800 rounded-lg border border-dashed border-gray-300 dark:border-gray-700"
					>
		...
		>
			<div class="flex items-center gap-3">
				...
				<div class="text-sm font-normal">
					Added {addedCount}
					{addedCount === 1 ? "book" : "books"} to collection!
				</div>
			</div>
		</Toast>
	{/if}
