...
>
	<div class="absolute inset-0 w-full h-full">
		{#if imageLoadFailed}
			<div
				class="flex items-center justify-center w-full h-full bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500"
			>
	...
	<Dropdown simple triggeredBy=".{dropdownId}" class="w-44 z-50">
		<DropdownItem onclick={handleToggleFavorite} class="flex items-center gap-2">
			{#if book.is_favorite}
		...
		<DropdownItem href={`/library/books/${book.id}/edit`} class="flex items-center gap-2">
	...
	<div
		class="absolute bottom-0 left-0 right-0 z-10 flex flex-col justify-end
                p-2 sm:p-3
	...
	>
		<h5
			class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white line-clamp-2 leading-tight mb-2 text-left"
			title={book.title}
		>
			{book.title}
		</h5>
		...
		<div class="flex flex-col gap-1.5">
			<div class="flex items-center justify-between">
				<Badge
					color={getStatusColor(book.reading_status)}
					class="px-1.5 py-0.5 text-[10px] border-0"
				>
